...
class TimeSpacePlotWidget(QWidget):
    ...
    def __init__(self):
        ...
        self._update_interval_ms = 100  # Default update interval in milliseconds
    ...
    def _create_plot_area(self):
        ...
        if view:
            ...
            # Try to set basic axis labels directly on the view
            try:
                if hasattr(view, 'setLabel'):
                    view.setLabel('bottom', 'Time (s)', color='k', **{'font-family': 'Times New Roman', 'font-size': '8pt'})
                    view.setLabel('left', 'FBG Index', color='k', **{'font-family': 'Times New Roman', 'font-size': '8pt'})
                    log.info("Successfully set basic axis labels on view")
            except Exception as e:
                log.debug(f"Could not set labels on view: {e}")
    ...
    def _get_plot_item_robust(self):
        ...
        try:
            # Method 1: Standard getView() method - most reliable for newer PyQtGraph
            view = self.image_view.getView()
            if view is not None:
                # Check if view is already a PlotItem
                if hasattr(view, 'setLabel') and hasattr(view, 'getAxis'):
                    plot_item = view
                    log.debug("Got PlotItem via getView() - direct PlotItem")
                    return plot_item
                # Check if view has getPlotItem method
                elif hasattr(view, 'getPlotItem'):
                    plot_item = view.getPlotItem()
                    if plot_item:
                        log.debug("Got PlotItem via getView().getPlotItem()")
                        return plot_item

            # Method 2: Try direct access to plotItem attribute
            if hasattr(self.image_view, 'plotItem'):
                plot_item = self.image_view.plotItem
                if plot_item and hasattr(plot_item, 'setLabel'):
                    log.debug("Got PlotItem via direct plotItem attribute")
                    return plot_item

            # Method 3: UI interface access (common in older versions)
            if hasattr(self.image_view, 'ui') and self.image_view.ui:
                ui = self.image_view.ui
                # Try graphicsView.plotItem
                if hasattr(ui, 'graphicsView') and ui.graphicsView:
                    gv = ui.graphicsView
                    if hasattr(gv, 'plotItem'):
                        plot_item = gv.plotItem
                        log.debug("Got PlotItem via UI.graphicsView.plotItem")
                        return plot_item
                    elif hasattr(gv, 'getPlotItem'):
                        plot_item = gv.getPlotItem()
                        if plot_item:
                            log.debug("Got PlotItem via UI.graphicsView.getPlotItem()")
                            return plot_item

        except Exception as e:
            log.warning(f"Error in _get_plot_item_robust: {e}")
    ...
    def _set_fallback_labels(self, data_shape: tuple):
        """Fallback method to set labels when PlotItem is not accessible."""
        try:
            original_time_points, original_spatial_points = data_shape
            scan_rate_hz = self._scan_rate if self._scan_rate > 0 else 50000
            time_duration_s = original_time_points / scan_rate_hz

            # Set image coordinate mapping - this is the most important part
            ...
            if image_item:
                ...
                log.info(f"Applied fallback image coordinates: Time=[0,{time_duration_s:.2f}]s, FBG=[{distance_start_actual},{distance_end_actual}]")

            # Try alternative axis setting methods
            view = self.image_view.getView()
            if view and hasattr(view, 'setLabel'):
                try:
                    view.setLabel('bottom', f'Time (s, max: {time_duration_s:.2f}s)',
                                 color='k', **{'font-family': 'Times New Roman', 'font-size': '8pt'})
                    view.setLabel('left', f'FBG Index ({distance_start_actual}-{distance_end_actual})',
                                 color='k', **{'font-family': 'Times New Roman', 'font-size': '8pt'})
                    log.info("Successfully set fallback axis labels")
                except Exception as e:
                    log.debug(f"Could not set fallback labels: {e}")

            log.info("Partial axis setup completed - coordinate mapping applied")
        except Exception as e:
            log.error(f"Fallback labeling failed: {e}")
            log.warning("Partial axis setup completed - may not show full ticks")
    ...
    def _update_axis_labels(self, data_shape: tuple):
        ...
        try:
            ...
            if plot_item is None:
                ...
                # Use fallback: Set basic title on ImageView
                self._set_fallback_labels(data_shape)
                return
