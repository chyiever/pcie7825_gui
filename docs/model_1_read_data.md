...
根据代码分析和用户需求，正确的数据流应该为：

```
1. 接收原始数据：(1, 228352) = (1, 223×1024)

2. 数据重组：reshape为(223, 1024) = (FBG数, 时间帧数)
   - 原始布局：[f0_fbg0, f0_fbg1, ..., f0_fbg222, f1_fbg0, ...]
   - 重组为2D：[[f0_fbg0, f1_fbg0, ...], [f0_fbg1, f1_fbg1, ...], ...]

3. 空间范围提取：[20:100, :] → (80, 1024)

4. 多帧水平拼接：5帧拼接 → (80, 5120)

5. 时间/空间降采样：2倍降采样 → (40, 2560)

6. 显示转置：转置为(2560, 40)以正确显示方向

7. 显示映射：
   - X轴：时间，范围[0, 5×1024/scan_rate]秒
   - Y轴：FBG索引，范围[20, 100]
```
...
2D数据(223,1024)
    ↓ 缓冲多帧
多帧缓冲[(223,1024), (223,1024), ...]
    ↓ 空间范围提取
空间窗口[(80,1024), (80,1024), ...]
    ↓ 水平拼接
拼接数据(80, 5120)
    ↓ 降采样
下采样数据(40, 2560)
...
**关键修复**：
1. **滚动方向**：添加转置操作确保正确的显示方向
2. **坐标映射**：X轴从0开始，Y轴对应FBG索引范围
3. **颜色映射**：修复colormap名称与实际颜色的对应关系
4. **显示样式**：白色背景颜色条，黑色坐标轴刻度
